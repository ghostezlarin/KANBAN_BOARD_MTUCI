...
def create_project(name: str):
    ...
    if error_code == 0:
        try:
            ...
            insert_query = """INSERT INTO t_projects (name, status) VALUES (%s, %s) RETURNING id"""
    ...
    return {client_id, error_code, error_desc}
...
def read_project(id: int):
    ...
    if error_code == 0:
        try:
            cursor = connection.cursor()
            insert_query = """SELECT name FROM public.t_projects WHERE id = %s AND status = 1 ORDER BY id;"""  # поправить статус
    ...
    return {name, error_code, error_desc}
...
def update_project(id: int, name: str):
    ...
    if error_code == 0:
        try:
            cursor = connection.cursor()
            insert_query = """UPDATE public.t_projects SET name = %s WHERE id = %s"""
    ...
    return {t, error_code, error_desc}
...
def delete_project(id: int):
    ...
    if error_code == 0:
        try:
            cursor = connection.cursor()
            insert_query = """UPDATE public.t_projects SET status = -1 WHERE id = %s"""
            record_to_query = (id,)
