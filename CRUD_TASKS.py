...
def create_task(name: str, project_id: int, user_id: int):
    connection = 0
    task_id: int = -1
    error_code = 0
    error_desc = ""
    ...
    if error_code == 0:
        try:
            ...
            insert_query = """INSERT INTO t_tasks (name, status, project_id, user_id) VALUES (%s, %s, %s, %s) RETURNING id"""
            record_to_insert = (name, "1", project_id, user_id)
...
def read_task(id: int):
    ...
    if error_code == 0:
        try:
            cursor = connection.cursor()
            insert_query = """SELECT name FROM public.t_tasks WHERE id = %s AND status = 1 ORDER BY id;"""  # поправить статус
...
def update_task(id: int, name: str):
    ...
    if error_code == 0:
        try:
            cursor = connection.cursor()
            insert_query = """UPDATE public.t_tasks SET name = %s WHERE id = %s"""
...
def delete_task(id: int):
    ...
    if error_code == 0:
        try:
            cursor = connection.cursor()
            insert_query = """UPDATE public.t_tasks SET status = -1 WHERE id = %s"""
            record_to_query = (id,)
